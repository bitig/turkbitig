document.addEventListener('DOMContentLoaded', () => {
  ...
  gokturk.style.overflowY = 'hidden';
  gokturk.style.resize = 'none';
  ...
  function convertToOldTurkic(input) {
    ...
    const rules = [
      ...
      { regex: /𐰃𐰴/gu, repl: '𐰶' },
      { regex: /𐰆𐰴/gu, repl: '𐰸' },
      { regex: /(?<=\S𐰀|𐰀\S)𐰀(?=[^\s\x00-\x7F])/gu, repl: '' },
      { regex: /(?<=\S𐰆|𐰆\S)𐰆(?=\S)/gu, repl: '' },
      { regex: /(?<=\S𐰃|𐰃\S)𐰃(?=\S)/gu, repl: '' },
      { regex: /(?<=\S𐰇|𐰇\S)𐰇(?=\S)/gu, repl: '' },
  ...
  function resizeTextarea() {
    gokturk.style.height = 'auto';
    const sh = gokturk.scrollHeight;
    if (sh > maxHeight) {
      gokturk.style.height = `${maxHeight}px`;
      gokturk.style.overflowY = 'auto';
    } else {
      gokturk.style.height = `${sh}px`;
      gokturk.style.overflowY = 'hidden';
    }
  }
  function scrollToCaret() {
    if (gokturk.style.overflowY === 'hidden') return;
    const text = gokturk.value;
    const start = gokturk.selectionStart;
    ...
    measurer.style.width = `${gokturk.clientWidth}px`;
    measurer.style.padding = getComputedStyle(gokturk).padding;
    measurer.style.font = getComputedStyle(gokturk).font;
    measurer.style.lineHeight = getComputedStyle(gokturk).lineHeight;
    measurer.style.letterSpacing = getComputedStyle(gokturk).letterSpacing;
    measurer.style.border = getComputedStyle(gokturk).border;
    measurer.textContent = textBeforeCaret.replace(/\n$/, '\n\u200B');
    document.body.appendChild(measurer);
    const heightToCaret = measurer.scrollHeight;
    document.body.removeChild(measurer);
    const lineHeight = parseFloat(getComputedStyle(gokturk).lineHeight) || 16;
    const topOfCaretLine = heightToCaret - lineHeight;
    const currentScrollTop = gokturk.scrollTop;
    const viewportHeight = gokturk.clientHeight;
    if (topOfCaretLine < currentScrollTop) {
      gokturk.scrollTop = topOfCaretLine;
    } else if (heightToCaret > currentScrollTop + viewportHeight) {
      gokturk.scrollTop = heightToCaret - viewportHeight;
    }
  }
  gokturk.addEventListener('beforeinput', (e) => {
    ...
    for (let m = 0; m < posMap.length; m++) {
      if (posMap[m] >= newInputPos) { newOutputPos = m; break; }
    }
