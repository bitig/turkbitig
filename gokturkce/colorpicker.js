...
const COLOR_CELL_WIDTH = 12; //  width 
const COLOR_CELL_HEIGHT = 10; // height 
const GRID_COLS = 8;        // grid columns
const GRID_ROWS = 6;        // grid rows
...
function generateColors() {
  ...
  for (let row = 0; row < GRID_ROWS; row++) {
    for (let col = 0; col < GRID_COLS; col++) {
      ...
      let s = 60 + Math.round((row / (GRID_ROWS - 1)) * 40); // saturation
      let v = 40 + Math.round((row / (GRID_ROWS - 1)) * 60); // brightness
